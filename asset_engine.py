...
- Detailed Performance Metrics, After-Tax Modeling, and Daily Weights Timeline
"""
...
def run_portfolio_backtest(
    ...
    """
    Simulates dynamic asset allocation strategy with conditional rebalancing and tax/fee modeling.
    """
    if dataset is None:
    ...
    for day in range(num_days):
        ...
        if matched_drop_stage is not None:
            ...
            if candidate_state != current_state_name and (days_since_rebalance >= cooldown_days or current_state_name == "평시 (Normal)"):
                ...
                trigger_reason = f"[낙폭 트리거 도달] {current_bm_dd:.1f}% (기준 {matched_drop_stage['threshold']}%)"
                new_state_name = candidate_state
                
        # B) Check Staircase Recovery Stages
            ...
            for rec_stage in recovery_stages:
                if current_bm_dd >= rec_stage["recovery"] and deepest_drop_level <= rec_stage.get("fromDrop", -20.0):
                    ...
                    if candidate_state != current_state_name and days_since_rebalance >= cooldown_days:
                        ...
                        trigger_reason = f"[반등 회복 트리거 도달] {current_bm_dd:.1f}% (저점 {deepest_drop_level:.1f}%)"
                        new_state_name = candidate_state
                        break
                        
            if current_bm_dd >= -5.0 and current_state_name != "평시 (Normal)":
                target_weights = normalize_weights(base_weights)
                trigger_reason = f"[대세 상승 복귀] {current_bm_dd:.1f}% -> 평시 비중 원복"
                new_state_name = "평시 (Normal)"
                deepest_drop_level = 0.0

        # C) Check Gain / Tolerance-Drift Rebalancing
        if target_weights is None and current_state_name == "평시 (Normal)" and days_since_rebalance >= 20:
            ...
            if gain_threshold_pct > 0 and nav_gain_pct >= gain_threshold_pct:
                target_weights = normalize_weights(base_weights)
                trigger_reason = f"[자산 성장 이익실현] 총자산 +{nav_gain_pct:.1f}% 상승 -> 목표 비중 재정렬"
                new_state_name = "평시 (이익실현 리밸런싱)"
            elif tolerance_band_pct > 0 and max_drift_pct >= tolerance_band_pct:
                target_weights = normalize_weights(base_weights)
                trigger_reason = f"[비중 이탈 보정] 최대 괴리율 {max_drift_pct:.1f}%p -> 기본 비중 환원"
        ...
        if target_weights is not None:
            ...
            rebalance_events.append({
                ...
                "nav": round(float(net_nav), 0),
                "weights": {format_ticker_display(t): round(target_weights.get(t, 0.0) * 100.0, 1) for t in target_weights.keys()},
    ...
    chart_strat_nav = [round(float(v), 0) for v in strategy_nav[::step]]
    chart_strat_dd = [round(float(v), 2) for v in strat_dd[::step]]
    
    chart_benchmarks = {}
    for bm_key, bm_nav in benchmarks.items():
        chart_benchmarks[bm_key] = [round(float(v), 0) for v in bm_nav[::step]]
    ...
    return {
        ...
        "finalNAV": round(float(strategy_nav[-1]), 0),
        "chartData": {
            "dates": chart_dates,
...
def get_default_strategy_config():
    return {
        ...
        "feeRate": 0.0015,
        "taxRate": 0.22,
        "taxDeduction": 2500000.0 # 연 250만원 공제
    }

def format_krw_str(val):
    if val >= 100_000_000:
        return f"{val / 100_000_000:.2f}억원 ({val:,.0f}원)"
    elif val >= 10_000:
        return f"{val / 10_000:,.0f}만원 ({val:,.0f}원)"
    return f"{val:,.0f}원"
...
if __name__ == "__main__":
    ...
    for ev in res['rebalanceEvents'][-5:]:
        print(f"  [{ev['date']}] {ev['reason']} (평가액: {format_krw_str(ev['nav'])}) -> {ev['weights']}")
